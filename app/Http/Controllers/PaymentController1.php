...
class PaymentController extends Controller
{
	...
	public function payment(Request $request)
	{
		...
		if ($paymentMode != 'CASH') {
			...
			if($tip_amount && $tip_amount > 0 && $fleet_user_id != $fleet_provider_id){
				if($fleet_user_id == '0'){
		...
		try {
			...
			if ($Requests->request_from == 'provider') {
				...
				if($Requests->from_id != '0'){
					...
					$stripe_acc_id = $fleet->stripe_cust_id;
					$email = $fleet->email;
				}
				else{
					// Card::where('user_id', 1)->value('card_id');
					$admin = Admin::where('id', 1)->first();
					$stripe_acc_id = Card::where('user_id', 0)->value('card_id');
					$email = $admin->email;
				}
			}
	...
	public function response(Request $request)
	{
		...
		$user_data = \App\User::find($log->user_id);
		switch ($log->payment_mode) {
			...
			case 'PAYTM':
				...
				break;
			case 'WALLET': 
				// if($user->allow_negative == 1 && $user->wallet_balance < $log->amount){
				// 	$balance = $user->wallet_balance - $log->amount;
				// 	User::where('id', $log->user_id)->update(['wallet_balance'=>$balance]);
				// }
				break;
		}
