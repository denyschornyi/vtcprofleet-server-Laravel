...
class FleetPointInterestResource extends Controller {
	...
	public function index( Request $request ) {
		$point_interest = DB::table( 'fleet_point_interests' )
			->join( 'fleet_poi_categories',
				'fleet_poi_categories.id',
				'=',
				'fleet_point_interests.start_poicategory_id' )
			->join( 'fleet_poi_categories as pc',
				'pc.id',
				'=',
				'fleet_point_interests.dest_poicategory_id' )
			->select( 'fleet_point_interests.*',
				'fleet_poi_categories.type as start_type',
				'pc.type as destination_type' )
			->where( [
				'fleet_poi_categories.status'    => '1',
				'pc.status'                      => '1',
				'fleet_point_interests.fleet_id' => Auth::guard( 'fleet' )->id(),
			] )->orderBy( 'fleet_point_interests.id', 'desc' )->get();
		if ( $request->ajax() ) {
			return $point_interest;
		} else {
			return view( 'fleet.point-interest.index',
	...
	public function create() {
		...
		$poi_category = FleetPoiCategory::where( [
			'status'   => 1,
		...
		$service_type =
			ServiceType::select( 'id', 'name' )->where( [ 'fleet_id' => 0 ] )
				->get();
		$shape_data   = FleetPolygonShape::where( [
			'fleet_id' => Auth::guard( 'fleet' )->id(),
		] )->get();

		return view( 'fleet.point-interest.create',
	...
	public function store( Request $request ) {
		...
		try {
			...
			return redirect()->route( 'fleet.pointInterest.index' )
				->with( 'flash_success',
	...
	public function show( $id ) {
		try {
			...
			return view( 'fleet.point-interest.show',
	...
	public function edit( $id ) {
		try {
			...
			$service_types    = ServiceType::select( 'id', 'name' )
				->where( [ 'fleet_id' => 0 ] )->get();
			$shape_data       = FleetPolygonShape::where( [
				'fleet_id' => Auth::guard( 'fleet' )->id(),
			] )->get();

			return view( 'fleet.point-interest.edit',
				compact( 'point_interest',
					'service_type',
					'service_types',
					'shape_data',
	...
	public function getShapeData() {
		$data =
			FleetPolygonShape::where( 'fleet_id', Auth::guard( 'fleet' )->id() )
				->get();
