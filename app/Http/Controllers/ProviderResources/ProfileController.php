...
class ProfileController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            Auth::user()->fleet = Fleet::find($fleet_id);

            Auth::user()->currency = config('constants.currency', '$');

            if ($fleet_id == 0) { // admin's provider
            ...
            } else { // fleet's provider
                $FleetPaymentSettings = FleetPaymentSettings::where('fleet_id', $fleet_id)->first();

                Auth::user()->cash = $FleetPaymentSettings->cash_payment_status == 'yes' ? 1 : 0;
                Auth::user()->card = $FleetPaymentSettings->stripe_payment_status == 'yes' ? 1 : 0;

                Auth::user()->stripe_secret_key = $FleetPaymentSettings->stripe_secret_key;
                Auth::user()->stripe_publishable_key = $FleetPaymentSettings->stripe_publish_key;
                Auth::user()->stripe_currency = $FleetPaymentSettings->stripe_currency_format;

                Auth::user()->payumoney = $FleetPaymentSettings->payumoney_status == 'yes' ? 1 : 0;
                Auth::user()->paypal = $FleetPaymentSettings->paypal_status == 'yes' ? 1 : 0;
                Auth::user()->paypal_adaptive = $FleetPaymentSettings->paypal_adaptive_status == 'yes' ? 1 : 0;
                Auth::user()->braintree = $FleetPaymentSettings->braintree_status == 'yes' ? 1 : 0;
                Auth::user()->paytm = $FleetPaymentSettings->paytm_status == 'yes' ? 1 : 0;

                Auth::user()->payumoney_environment = $FleetPaymentSettings->payumoney_env == 'Development' ? 'test' : 'secure';
                Auth::user()->payumoney_key = $FleetPaymentSettings->payumoney_key;
                Auth::user()->payumoney_salt = $FleetPaymentSettings->payumoney_salt;
                Auth::user()->payumoney_auth = $FleetPaymentSettings->payumoney_auth;

                Auth::user()->paypal_environment = $FleetPaymentSettings->paypal_env == 'Development' ? 'sandbox' : 'live';
                Auth::user()->paypal_currency = $FleetPaymentSettings->paypal_currency_format;
                Auth::user()->paypal_client_id = $FleetPaymentSettings->paypal_client_id;
                Auth::user()->paypal_client_secret = $FleetPaymentSettings->paypal_client_secret;

                Auth::user()->braintree_environment = $FleetPaymentSettings->braintree_env == 'Development' ? 'sandbox' : 'live';
                Auth::user()->braintree_merchant_id = $FleetPaymentSettings->braintree_merchantid;
                Auth::user()->braintree_public_key = $FleetPaymentSettings->braintree_publishkey;
                Auth::user()->braintree_private_key = $FleetPaymentSettings->braintree_privatekey;
