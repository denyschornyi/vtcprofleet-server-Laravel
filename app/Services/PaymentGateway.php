...
class PaymentGateway {
	...
	public function process($attributes) {
		...
		switch ($this->gateway) {
			case "WALLET":
				return redirect($provider_url.'/payment/response?order='. $attributes['order']);
				break;
			case "BRAINTREE":
			...
			case "STRIPE":
				...
				try {
					...
					if (empty($attributes['stripe_secret_key'])) {
						Stripe::setApiKey(config('constants.stripe_secret_key', ''));
					} else {
						Stripe::setApiKey($attributes['stripe_secret_key']);
					}
					...
					if($log->user_type == 'user'){
						$not_paid_requests = UserRequests::where('user_id', Auth::user()->id)->where('status', 'COMPLETED')->where('paid', '0')->orderBy('created_at', 'asc')->pluck('id')->toArray();
						foreach($not_paid_requests as $val){
							$total += UserRequestPayment::where('request_id', $val)->value('total');
							if($total > $attributes['amount']) break;
							UserRequests::where('id', $val)->update(['paid' => '1']);
						}
					}
