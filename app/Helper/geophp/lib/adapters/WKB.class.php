...
class WKB extends GeoAdapter {
	...
	public function read( $wkb, $is_hex_string = false ) {
		...
		if ( empty( $wkb ) ) {
			throw new Exception( 'Cannot read empty WKB geometry. Found '
			                     . gettype( $wkb ) );
		}
	...
	function getLinstring( &$mem ) {
		...
		$line_coords = unpack( 'd*',
			fread( $mem, $line_length[1] * $this->dimension * 8 ) );
		...
		while ( $i <= $num_coords ) {
			$components[] =
				new Point( $line_coords[ $i ], $line_coords[ $i + 1 ] );
			$i            += 2;
		}
