...
class WKT extends GeoAdapter {
	...
	public function write( Geometry $geometry ) {
		...
		if ( \App\Helpers\geoPHP::geosInstalled() ) {
	...
	public function read( $wkt ) {
		...
		foreach ( \App\Helpers\geoPHP::geometryList() as $geom_type ) {
			$wkt_geom = strtoupper( $geom_type );
			if ( strtoupper( substr( $wkt, 0, strlen( $wkt_geom ) ) )
			     == $wkt_geom
			) {
