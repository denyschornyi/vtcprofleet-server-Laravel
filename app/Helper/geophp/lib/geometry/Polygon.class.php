...
class Polygon extends Collection {
	...
	public function pointInPolygon( $point, $c = false ) {
		foreach ( $this->components as $component ) {
			...
			$j   = $num - 1;
			for ( $i; $i < $num; $i ++ ) {
				if ( ( $pts[ $i ]->y() > $point->y() ) != ( $pts[ $j ]->y()
				                                            > $point->y() )
				     && ( $point->x() < ( $pts[ $j ]->x() - $pts[ $i ]->x() )
				                        * ( $point->y() - $pts[ $i ]->y() )
				                        / ( $pts[ $j ]->y() - $pts[ $i ]->y() )
				                        + $pts[ $i ]->x() )
				) {
	 ...
	 * @return boolean
	 */
	public function pointInPolygon_( $point,
