...
class UserRequests extends Model
{
	...
	{
		return $this->hasOne('App\Pool','request_id');
	}

	public function poolTransaction()
	{
		return $this->hasOne('App\PoolTransaction','request_id');
	}

	public function privatePoolRequests()
	{
		return $this->hasOne('App\PrivatePoolRequests','request_id');
	}
    ...
    public function scopeRequestHistory($query)
    {
        return $query->orderBy('user_requests.schedule_at', 'desc')
                        ->with('user','payment','provider','pool','poolTransaction','privatePoolRequests');
    }
