            ...
            <table class="table table-striped table-bordered dataTable" id="table-2">
                ...
                @foreach($polygon_shape as $index => $poi)
                    <tr>
                        ...
                        <td>{{ $poi->type }}</td>
                        <td>
                            <form action="{{ route('admin.polygonshape.destroy', $poi->id) }}" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                @if( Setting::get('demo_mode', 0) == 0)
                                    <a href="{{ route('admin.polygonshape.show', $poi->id) }}" class="btn btn-success">
                                        <i class="fa fa-book"></i> @lang('admin.poi.show')
                                    </a>
                                    <a href="{{ route('admin.polygonshape.edit', $poi->id) }}" class="btn btn-info">
