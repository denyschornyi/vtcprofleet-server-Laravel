                ...
                <div class="col-md-6">
                    <dl class="row">
                        ...
                        <dd class="col-sm-6" style="font-size: 17px;border-top: 2px solid;width: 11%;padding-top: 10px;">
                            ...
                            @if(($request->payment->fixed + $request->payment->minute + $request->payment->distance + $request->payment->waiting_amount + $request->payment->peak_amount) < $min_price)
                                {{ currency($min_price) }}
                            @else
                                {{ currency($request->payment->fixed + $request->payment->minute + $request->payment->distance + $request->payment->waiting_amount + $request->payment->peak_amount) }}
                            @endif
                        </dd>
                        ...
                        <dd class="col-sm-6" >
                            {{-- @if(!in_array($request->provider_id, $admin_provider_ids) && in_array($request->user_id, $admin_user_ids)) --}}
                                {{ currency($request->payment->pool_commission) }}
                            {{-- @else --}}
                                {{-- {{ currency(0.00) }} --}}
                            {{-- @endif --}}
                        </dd>
                        ...
                        {{-- @if(isset($statement_for) && $statement_for == 'admin')
                        <dd class="col-sm-6">@if(in_array($request->provider_id, $admin_provider_ids)){{ currency($request->payment->commision) }}@else{{ currency(0.00) }}@endif</dd>
                        @else --}}
                        ...
                        {{-- @if($request->payment->peak_comm_amount != 0)<dt class="col-sm-6">@lang('admin.request.peak_commission') :</dt>@endif
                        <dd class="col-sm-6">{{ currency($request->payment->peak_comm_amount) }}</dd>
                        
                        @if($request->payment->waiting_comm_amount != 0)<dt class="col-sm-6" style="padding-right:0px;margin-bottom: 17px;">@lang('admin.request.waiting_commission') :</dt>@endif
                        @if($request->payment->waiting_comm_amount != 0)<dd class="col-sm-6">{{ currency($request->payment->waiting_comm_amount) }}</dd>@endif --}}
