        ...
        <div class="container-fluid">
            ...
            <div class="box box-block bg-white">
                ...
                @can('lost-item-create')
                    <a href="{{ route('admin.userdisputecreate') }}" style="margin-left: 1em;"
                       class="btn btn-primary pull-right"><i class="fa fa-plus"></i> @lang('admin.dispute.add_dispute')
                    </a>
                @endcan
                ...
                <table class="table table-striped table-bordered dataTable" id="table-2">
                    ...
                    @foreach($disputes as $index => $dispute)
                        <tr>
                            ...
                            <td>@if($dispute->dispute_type=='user')
                                    @if($dispute->user != null) {{ $dispute->user->first_name." ".$dispute->user->last_name }} @endif
                                @else
                                    @if($dispute->provider != null)  {{ $dispute->provider->first_name." ".$dispute->provider->last_name }} @endif
                                @endif
                            </td>
                            ...
                            <td>
                                @if( Setting::get('demo_mode', 0) == 0)
                                    @can('dispute-edit')
                                        @if($dispute->status=='open')
                                            <a href="{{ route('admin.userdisputeedit', $dispute->id) }}" href="#"
                                               class="btn btn-info"><i class="fa fa-pencil"></i> View</a>
                                        @endif
                                    @endcan
                                @endif
                            </td>
