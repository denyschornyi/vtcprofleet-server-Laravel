...
<div class="content-area py-1">
    <div class="container-fluid">
        @include('common.fleet_b2b_history_date_filter', ['status' => $status,'fleet_id' => $take_id])
        <div class="box box-block bg-white">
            <h5 class="mb-1">@lang('admin.include.pool_history')</h5>
            @if(count($requests) != 0)
            <table class="table table-striped table-bordered dataTable" id="table-6">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            {{ $request->poolTransaction->commission }} %
                        </td>
                        <td>
                            @if($request->poolTransaction->commission != null)
                                {{ currency($request->total_price - ( $request->total_price * $request->poolTransaction->commission / 100 ) ) }}
                            @else
                                {{ currency($request->total_price) }}
                            @endif
                        </td>
