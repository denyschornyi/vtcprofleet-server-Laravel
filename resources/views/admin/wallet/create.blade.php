            ...
            <form class="form-horizontal" action="{{route('admin.transferstore')}}" method="POST" enctype="multipart/form-data" role="form" autocomplete="off">
				...
				<div class="form-group row">
					...
					<div class="col-xs-5">
						...
						<input type="hidden" name="wallet_balance" id = "balance_of_wal" value="" />
					</div>		
				</div>
...
$('#namesearch').autocomplete({
	            ...
	            if (!responsedata.data.length) {
	             ...
	             response( $.map(responsedata.data, function( item ) {
	                    if(sflag==1) {
							...
							var ballance = item.wallet_balance;
						}
						else{
							var name_alias=item.company;
							var credit = 0;
							var debit = 0;
							var res = item.pool;
							...
							result.forEach(element => {
								...
								var value_ary = element.split('_');
								
								if(element.includes("credit0")){
									credit = parseFloat(value_ary[1]);
								}
								if(element.includes("debit0")){
									debit = parseFloat(value_ary[1]);
								}
							});
							var ballance = debit - credit;
						}
	                    return {                                
	                        value: name_alias,
	                        id: item.id,                
	...
	select: function (event, ui) {    
		$("#balance_of_wal").val(ui.item.bal);    
