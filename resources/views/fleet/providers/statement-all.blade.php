		...
		<div class="box box-block bg-white">
			...
			<div style="text-align: center;padding: 20px;color: blue;font-size: 24px;">
				...
				<p><strong>
						...
						<br>
						<span>@lang('admin.dashboard.pool_commission') : {{currency($revenue['pool_commission'])}}</span>
						{{-- <br>
				...
				<div class="col-lg-4 col-md-6 col-xs-12">
					<div class="box box-block bg-white tile tile-1 mb-2">
						...
						<div class="t-content">
							...
							<i class="fa fa-caret-up text-success mr-0-5"></i><span>from {{$completed_ride_count}} Rides</span>
						</div>
					</div>
				</div>
				@else
				<div class="col-lg-4 col-md-6 col-xs-12">
					<div class="box box-block bg-white tile tile-1 mb-2">
						...
						<div class="t-content">
							...
							<h1 class="mb-1">{{currency($revenue['commission'])}}</h1>
							...
							<table class="table table-striped table-bordered dataTable" id="table-8">
								...
								<tbody>
									...
									<tr>
										...
										{{-- @if(isset($statement_for) && $statement_for !="user") --}}
										{{-- @if($ride->fleet_id != $loginedId)
											<td>{{currency($ride->payment->pool_commision)}}</td>
										@else
											<td>{{ currency($ride->payment->commision) }}</td>
                                        @endif --}}
                                        <td>
                                            @if((in_array($ride->user_id, $userIds) && in_array($ride->provider_id, $providerIds)) || !in_array($ride->user_id, $userIds))
                                                {{ currency($ride->payment->commision + $ride->payment->peak_comm_amount + $ride->payment->waiting_comm_amount) }}
                                            @else
                                                {{ currency(0.00) }}
                                            @endif
                                        </td>
										<td>
                                            {{-- @if((in_array($ride->user_id, $userIds) && !in_array($ride->provider_id, $providerIds) && $ride->payment_mode == 'CASH') || (!in_array($ride->user_id, $userIds) && in_array($ride->provider_id, $providerIds) && $ride->payment_mode=='CARD')) --}}
											@if(in_array($ride->user_id, $userIds) && !in_array($ride->provider_id, $providerIds))	
												{{ currency($ride->payment->pool_commission) }}
                                            @else
                                                {{ currency(0.00) }}
                                            @endif
                                        </td>
		...
		buttons: [{
		...
		},
			{
				...
				action: function ( e, dt, node, config ) {
					var statement_for = "fleet";
					var id = 0;
					var searchVal = $('.dataTables_filter input').val();
					var base = '{!! route('fleet.downloadExcel') !!}';
					var url = base + '?st=' + statement_for + '&id=' + id + '&searchVal='+searchVal ;
