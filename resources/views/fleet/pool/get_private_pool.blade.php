    ...
    <div class="content-area py-1">
        <div class="container-fluid">
            <div class="box box-block bg-white">
                ...
                <a href="{{ route('fleet.add.private_pool') }}" style="margin-left: 1em;" class="btn btn-primary pull-right">
                        ...
                        @foreach($pool_data as $key=>$val)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        {{ \App\Fleet::where('id',$val->from_fleet_id)->value('company') }}
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    @php
                                        ...
                                        $status = \App\PrivatePoolPartners::where(['pool_id'=>$val->id,'fleet_id'=>$val->from_fleet_id, 'action_id'=> \Illuminate\Support\Facades\Auth::user()->id])
                                        ->value('status');
                                        //get ride count of private pools
                                        $rideCount = \App\Pool::join('private_pool_requests','pools.request_id','=','private_pool_requests.request_id')
                                                    ->whereNull('pools.deleted_at')->where('private_pool_requests.private_id',$val->id)->count();
                                    @endphp
                                    @if($val->PrivatePoolID && ($val->from_fleet_id == $logined_fleet_id ) && $rideCount > 0)
                                        <a href="{{ route('fleet.open.private_pool',$val->id) }}" class="btn btn-info">
                                ...
                                <td>
                                    @if ($val->from_fleet_id == \Illuminate\Support\Facades\Auth::user()->id )
                                        <form action="{{ route('fleet.delete.private_pool', ['id'=>$val->id]) }}" method="POST">
                                            {{ csrf_field() }}
                                            <a href="{{ route('fleet.edit.private_pool', $val->id ) }}" class="btn btn-info">
                                        ...
                                        @endphp
                                            @if ($status == '0')
                                                <form action="{{ route('fleet.accept.private_pool', ['id'=>$val->id]) }}" method="POST" style="float: left;margin-right: 5px;">
                                                    {{ csrf_field() }}
                                                    <button class="btn btn-info">
                                                        <i class="fa fa-apple"></i> @lang('admin.fleets.accept')
                                                    </button>
                                                </form>
                                                <form action="{{ route('fleet.refuse.private_pool', ['id'=>$val->id]) }}" method="POST">
                                                    {{ csrf_field() }}
                                                    <button class="btn btn-danger" onclick="return confirm('Are you sure?')">
