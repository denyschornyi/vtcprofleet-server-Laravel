    ...
    <div class="content-area py-1">
        <div class="container-fluid">
            <div class="box box-block bg-white">
                <h5 class="mb-1">@lang('admin.payment.completed_payment') @if($from_date) From {{appDate($from_date)}} @endif @if($to_date) To {{appDate($to_date)}} @endif</h5>
                @php
                    if ($utype == 'provider') {
                        $action_route = 'fleet.payment_provider';
                    }
                    else {
                        $action_route = 'fleet.payment_fleet';
                    }
                @endphp
                @include('common.date_filter', ['action' => $action_route])
                ...
                <table class="table table-striped table-bordered dataTable" id="table-6">
                    ...
                    @foreach($payments as $index => $payment)
                        <tr>
                            ...
                            @else
                                <td>{{ $payment->fleet? $payment->fleet->company : 'Admin'}}</td>
                            @endif
                            <td>
